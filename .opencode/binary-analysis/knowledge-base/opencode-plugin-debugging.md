...
> BinaryAnalysis Plugin（`.opencode/plugins/binary-analysis.mjs`）的测试方法和问题排查流程。
...
```bash
node --check .opencode/plugins/binary-analysis.mjs
```
...
2. 发送一条消息
3. 观察：
   - 如果 `system.transform` 正常工作，Agent 的系统提示中应包含 `## BinaryAnalysis 环境信息` 段
...
1. 确保 `~/bw-ida-pro-analysis/config.json` 存在且有效
...
- Plugin 导出名称不是默认导出（应为 `export const BinaryAnalysisPlugin = ...`）
...
检查 Plugin 是否加载:
  4. .opencode/plugins/binary-analysis.mjs 是否存在？
  5. node --check 通过？
  6. 导出是否正确？（export const BinaryAnalysisPlugin = ...）
...
1. config.json 路径是否正确？
   → ~/bw-ida-pro-analysis/config.json
   → Windows: C:\Users\<用户名>\bw-ida-pro-analysis\config.json
...
2. COMPACTION_CONTEXT_PROMPT 内容是否包含关键信息？
   → 检查 binary-analysis.mjs 中的 COMPACTION_CONTEXT_PROMPT 常量
...
1. 确认 `~/bw-ida-pro-analysis/config.json` 和 `env_cache.json` 存在
...
| 问题 | 原因 | 解决 |
|------|------|------|
| Plugin 语法检查通过但不生效 | 导出名称不匹配 | 确认 `export const BinaryAnalysisPlugin` |
| 环境信息为空 | config.json 不存在 | 创建 `~/bw-ida-pro-analysis/config.json` |
| 压缩后规则丢失 | compacting hook 未注入 | 检查 output.context.push() 调用 |
| Agent 未在 Tab 列表中显示 | frontmatter 格式错误 | 检查 YAML --- 分隔符 |
| Agent 加载但不读知识库 | prompt 中引用路径错误 | 确认使用 `$SCRIPTS_DIR/knowledge-base/` |
