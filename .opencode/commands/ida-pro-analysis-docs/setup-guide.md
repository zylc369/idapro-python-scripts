...
成功输出应包含:
```json
{"success": true, "data": {"compiler": {"available": true, ...}, "packages": {...}}, "errors": []}
```
...
- [ ] 切换到 binary-analysis Agent 后，对话中能看到"BinaryAnalysis 环境信息"段（由 Plugin 注入）
...
| gmpy2 安装失败 (Windows) | 用 conda: `conda install -c conda-forge gmpy2` |
