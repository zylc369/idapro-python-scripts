...
3. 路径处理：
   - 绝对路径：直接使用
   - 相对路径：相对于项目根目录解析（用上方"项目根目录"的值拼接）
...
3. 验证通过后写入配置：
   ```bash
   mkdir -p .config && echo '{"ida_path":"<验证后的路径>"}' > .config/ida_config.json
   ```
4. 后续命令执行时自动使用配置路径
...
每次命令执行时，在项目根目录下创建 `analysis_intermediate/<timestamp>_<短hash>/` 子目录（如 `analysis_intermediate/20260417_143052_a1b2/`），所有中间文件均存放在此目录中。

```bash
TASK_DIR="analysis_intermediate/$(date +%Y%m%d_%H%M%S)_$(openssl rand -hex 2)"
mkdir -p "$TASK_DIR"
```
...
沉淀脚本位于 `<项目根>/.opencode/commands/ida-pro-analysis-scripts/scripts/` 目录下，调用方式与核心脚本相同：

```bash
IDA_FUNC_ADDR=<地址> IDA_OUTPUT="$TASK_DIR/result.json" \
  "<IDA_PATH>/idat" -A -S"<项目根>/.opencode/commands/ida-pro-analysis-scripts/scripts/<脚本名>.py" \
  -L"$TASK_DIR/idat.log" "<目标文件>"
```
