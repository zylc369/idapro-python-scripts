...
2. **Plugin 按 Agent 注入**不同环境信息：
   ...
   - mobile-analysis：移动端工具路径（从 config.json tools 按 agents 字段过滤）+ IDA Pro 路径 + Python 包
3. **config.json 重构**：删除 `scripts_dir`（由 Plugin 从 agent 名动态推导）；tools 值从"路径字符串"升级为"数据结构"（含 agent 过滤、必选标记、版本检测命令）
4. **detect_env.py 重构**：不再硬编码 MOBILE_TOOLS 字典，改为读取 config.json tools 配置，按 `version_cmd` 检测可用性，支持 `--agent` 参数按需检测
...
│   ├── update.py
│   ├── scripts/
│   │   ├── detect_env.py         # 改造：从 config.json 读取工具配置，不再硬编码 MOBILE_TOOLS
...
```json
{
  ...
  "tools": {
    ...
    "class-dump": {
      "path": "/usr/local/bin/class-dump",
      "agents": ["mobile-analysis"],
      "required": false,
      "version_cmd": ["class-dump", "--help"],
...
```
流程：
1. 从 input.sessionID 查 sessionAgentMap → 获取 agentName
2. agentName 为 undefined → 注入通用环境信息（IDA 路径 + 全部 tools + BA_PYTHON + packages）
...
| `$IDA_SCRIPTS_DIR` | 不注入（=`$SCRIPTS_DIR`） | `.opencode/binary-analysis/` | Plugin 动态推导 |
| 编译器 | 注入 | 不注入 | env_cache.json compiler |
| Python 包 | 注入（全部） | 注入（frida，unicorn） | env_cache.json packages |
...
3. 恢复 $SCRIPTS_DIR、$TASK_DIR 等关键变量`;
}
```
...
Plugin 在 compacting 和 system.transform 中注入设备状态路径提示。
...
// 注：tools 字段替代原来的 mobile_tools，包含 config.json 中所有工具的检测结果
```

### 2.4 detect_env.py 重构

**核心变更：不再硬编码 MOBILE_TOOLS 字典，改为从 config.json 读取工具配置。**

```python
def _detect_tools(config):
    ...
    for name, cfg in tools.items():
        ...
        if os.path.isfile(path) and os.access(path, os.X_OK):
            version = _get_tool_version(path, version_cmd)
...
def _get_tool_version(path, version_cmd):
    """执行 version_cmd 获取版本字符串"""
    if not version_cmd:
        return "unknown"
    try:
        r = subprocess.run([path] + version_cmd[1:], capture_output=True, text=True, timeout=10)
        if r.returncode == 0:
...
| `binary-analysis/scripts/detect_env.py` | **修改** | 删除 MOBILE_TOOLS 硬编码，改为读取 config.json tools，新增 `--agent` 参数，工具检测结果写入 env_cache.json data.tools |
...
步骤 5. 创建移动端 Frida 知识库
  ...
  - 内容要点:
    ...
    3. **device.json 规范**：任务目录下的设备状态文件结构和用途
...
步骤 9. 重构 detect_env.py
  ...
  - 预估行数: ~100 行改动
  - 改动内容:
    1. 删除 MOBILE_TOOLS 字典硬编码
    2. 新增 _detect_tools(config) 函数：读取 config.json tools → 逐工具执行 version_cmd → 输出可用性+版本
    3. 新增 _get_tool_version(path, cmd) 函数：执行 cmd 捕获版本字符串
    4. 新增 --agent 参数：指定 agent 名时只检测该 agent 的 tools（按 tools[].agents 过滤）
    5. agent 未知 → 检测全部
    6. 工具检测结果写入 env_cache.json data.tools 字段
  ...
  - 验证点:
    ...
    4. env_cache.json 输出包含 data.tools 字段，结构正确
    5. 已有工具逻辑（venv、compiler、packages、ida_pro）不受影响
...
步骤 10. 改造 Plugin — 按 Agent 注入 + 动态 compacting
  ...
  - 改动内容:
    ...
    2. EnvData 接口：tools 字段替代原 mobile_tools
    3. 新增 AGENT_SCRIPT_DIRS 常量
    4. 新增 getToolsForAgent(agentName, config) 过滤函数
    5. 新增 getCompactionReminder(agentName) 动态生成函数（含 undefined→用户确认逻辑）
    6. 新增 getCompactionContext(agentName) 通用部分+动态追加函数
    7. system.transform：查 sessionAgentMap → 分支注入（环境信息 + tools 过滤 + 脚本目录推导）
    8. compacting：按 agent 使用对应的 COMPACT_REMINDER 和 compaction context，TASK_DIR 恢复仍需共用逻辑
    9. 删除所有 scripts_dir 相关逻辑
  - 验证点:
    1. `node --check` 编译通过
    2. binary-analysis 注入环境信息与改造前一致（回归）
...
步骤 11. 更新 binary-analysis.md 变量初始化
  ...
  - 预估行数: ~20 行改动
  - 改动内容:
    1. bash fallback：删除 `c.get('scripts_dir', ...)` 中的 config.json 读取，直接用项目结构推导
    2. PowerShell fallback：同理
  - 验证点:
    1. 不再引用 config.json 的 scripts_dir
    2. fallback 正确推导到 .opencode/binary-analysis/
    3. Plugin 注入的 $SCRIPTS_DIR 优先级高于 fallback
...
步骤 12. 更新环境文档
  ...
  - 改动内容:
    ...
    5. setup-guide.md: config.json 模板更新为结构化格式，新增 mobile-analysis Agent 说明
...
| F3 | IPK 多路径选择 | Agent 能根据分析需求选择正确路径（Java 逻辑 / smali / native / Hybrid） |
...
| F8 | 工具配置统一 | config.json tools 为唯一配置源，detect_env.py 和 Plugin 均从此读取，无硬编码 MOBILE_TOOLS |
