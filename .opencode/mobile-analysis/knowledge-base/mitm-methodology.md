...
Interceptor.attach(libc.getExportByName("getaddrinfo"), {
    onEnter: function(args) {
        ...
        if (host === targetHost) {
            ...
            args[0] = Memory.allocUtf8String(proxyIP);
            console.log("[DNS] Redirected " + targetHost + " → " + proxyIP);
        }
    }
});
```
        ...
        try {
            ...
            if (family === 2) {  // AF_INET
                ...
                if (currentPort === targetPort) {
                    sockaddr.add(2).writeU8(proxyPort >> 8);       // 高字节
                    sockaddr.add(3).writeU8(proxyPort & 0xFF);     // 低字节
                    console.log("[NET] Port redirect: " + targetPort + " → " + proxyPort);
                }
            }
        } catch(e) {}
    }
