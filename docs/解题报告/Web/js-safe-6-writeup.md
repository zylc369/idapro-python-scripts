...
- check() 的 `while(true) debugger` 条件不满足 → 不执行 → 不暂停
...
| 4 | check() 自检 | 检测 check 源码是否被修改 → 无限 debugger 卡死浏览器 | 无（防篡改，不影响 step） |
...
| 角色 | 机制 | 说明 |
|------|------|------|
| **干活的主力** | 1、2、3 | 直接累加 `window.step`，是 flag 校验算法的核心组成部分 |
| **看门护院的保镖** | 4、5 | 不参与 step 计算，但阻止你篡改代码——如果你改了 check 的源码（机制 4）或改了 HTML 文件（机制 5），程序会卡死或暂停，让你无法继续 |
| **障眼法和暗门** | 6、7、8 | 不参与 step 计算，但干扰你的分析——让你误以为 step 会被翻倍（机制 6）、让你看不清变量名（机制 7）、让你看不出 pool 被做了 ROT13 变换（机制 8） |

换句话说：只有机制 1/2/3 在"算数"，机制 4/5 保证你没法改算法，机制 6/7/8 让你很难看懂算法。**它们是一个整体**——没有保镖（4/5），你可以直接改掉 check 函数跳过校验；没有障眼法（6/7/8），算法一眼就能看穿。
...
这道题前后花了一周、写了 300+ 个版本的脚本。回头看，每个机制都曾让我在某个方向上浪费了大量时间。按机制逐个复盘，希望能帮后来者少走弯路。

#### 机制 1：instrument() 的 debug condition 累加 step

**我踩的坑：试图绕过 debug condition**

一开始我认为 debug condition 是"反调试障碍"，应该绕过。具体做法是：不调用 `anti(debug)`，而是手动用 Python 模拟 `instrument()` 的效果——在 JavaScript 外面计算每个函数的源码长度，然后手动累加 step。

**为什么失败了？** 因为 debug condition 的执行上下文和直接调用不同。在 debug condition 中调用其他被 instrument 的函数时，Chrome 会**抑制嵌套的 condition 评估**（见 [1.2 什么是 debug condition](#12-什么是-debug-condition调试条件)）。这意味着同一个函数在 debug condition 上下文中和在普通上下文中累加的 step 值不同。我在这个差异上花了大量时间（几十个版本），试图精确测量两种上下文的 step 增量差异，但每次测量都有细微偏差。

**正确的做法**：根本不要绕过 debug condition。让 Chrome 正常执行它，step 自然就是正确的。最终成功的 v333 版本就是老老实实调用 `anti(debug)`，让所有 debug condition 正常运行。

**教训**：不要和运行环境对着干。题目设计的机制就是要在 Chrome 中正常运行的，你在 Chrome 外模拟它，必然会有差异。

#### 机制 2：instrumentPrototype() 的 getter 劫持

**我踩的坑：没理解 getter 返回旧值导致的边界问题**

getter 中的 `step++` 返回旧值。当 step=0 时，`0 && 原始函数` = 0，getter 返回的是 0 而不是函数。这意味着 step 被重置为 0 后，第一次访问数组方法会失败。

我在模拟 step 增量时，统一给每次数组方法访问加 1，没有考虑 step=0 这个边界条件。后来发现 check() 函数在重置 step 后故意先执行 `[0].step`（触发 Proxy 使 step 变为 1），就是为了避开这个边界。但我在模拟时漏掉了这一步。

**教训**：读代码不能只看"累加逻辑"，还要看边界条件和初始化顺序。getter 的 `++` 返回旧值是 JS 基础知识，但在复杂上下文中很容易忽略它的副作用。

#### 机制 3：instrumentPrototypeOfPrototype() 的 Proxy 拦截

**我踩的坑：低估了 Proxy 的拦截范围**

Proxy 拦截的是 `Array.prototype` 的原型（即 `Object.prototype`）。看起来只影响数组操作，但实际上 JavaScript 中几乎所有操作都会涉及原型链查找——包括 `Function.prototype` 上的方法调用、`Object.prototype` 上的 `toString()` 等。

我在模拟时只给数组方法加了 Proxy 的 step 增量，忽略了其他操作也会触发 Proxy。这导致模拟的 step 值总是比实际少。

**教训**：Proxy 一旦插入原型链，影响范围比你想象的大得多。不要假设"只有数组操作才触发"。

#### 机制 4：check() 自检（源码长度校验）

**我踩的坑：把自检当成了必须绕过的障碍**

看到 `while(true) debugger`，我的第一反应是"必须绕过这个检查"。于是尝试了：
- 用 `toString()` 替换 check 函数
- 用 `Object.defineProperty` 修改函数的 toString
- 重写 check 函数保持长度不变

**实际上完全不需要绕过**。自检只在 step!=0 或 check 源码被修改时才触发。正常执行 `anti(debug)` 后再执行 `unlock(flag)`，check 的源码没有被修改，step 也在正常范围内——自检根本不会触发。我花了很多时间在一个不会触发的问题上。

**教训**：先搞清楚一个防御机制是否真的会触发，再决定是否需要处理它。"防篡改"机制只在篡改时才生效，正常使用时它是透明的。

#### 机制 5：HTML 长度校验

**我踩的坑：试图修改 HTML 文件来简化分析**

一开始我想删掉 HTML 中的动画部分（ASCII 立方体），让代码更短更容易读。但删完后发现所有被 instrument 的函数都触发了断点——因为 HTML 长度不再是 14347。

然后我又想"那我把 HTML 长度补齐到 14347"，加了一堆注释和空格。结果发现注释中的 Unicode 字符影响了其他逻辑。折腾了几个小时后放弃了。

**教训**：不要修改目标文件。即使你理解了某个检查的原理，修改文件也可能引入意想不到的副作用。保持原始文件不变，在运行时层面操作。

#### 机制 6：double() 障眼法

**我踩的坑：花了大量时间分析 double() 的作用**

看到 `window.step[U+FFA0] *= 2`，我认为：
1. double() 会把 step 乘以 2
2. 乘以 2 的时机很重要，必须在 check() 循环的某个特定点
3. 我需要精确模拟这个翻倍操作

于是写了很多版本的脚本，有的在每次匹配后手动 `step *= 2`，有的在特定条件下翻倍。每个版本算出的 flag 都不对。

后来仔细分析才发现：`Function.call` + tagged template 创建的是空函数体（见 [3.7 机制六](#37-机制六double-障眼法)），double() 调用后**什么都不做**。但发现这一点之前，我在 double() 上至少浪费了一天。

**教训**：当代码看起来"应该有作用"但实际运行结果和预期不符时，不要假设是你模拟错了——**可能是代码本身就不做你以为的事**。用 `toString()` 检查函数体是最直接的验证方式。

#### 机制 7：U+FFA0 隐藏标识符

**我踩的坑：混淆 window.step 和 window.step[U+FFA0]**

源码中有两组极其相似的变量名：
- `window.step` — 真正的步数计数器
- `window.step[U+FFA0]`（step 后面跟一个不可见的 U+FFA0）— 另一个变量，和 step 无关

在很多版本中，我把 `window.step[U+FFA0] *= 2`（double() 里）当成了 `window.step *= 2`，认为 step 会被翻倍。后来才注意到 U+FFA0 的存在。

更严重的是 `window.c`（DOM 元素，truthy，来自 `<meta id="c">`）和 `window.cﾠ`（布尔值 true，由 anti() 设置）的混淆。这两个变量名只差一个不可见的 U+FFA0 字符。debug condition 里写的是 `window.c`（无 U+FFA0），而 anti() 设置的是 `window.cﾠ`（有 U+FFA0）——幸好 `window.c` 是 truthy（DOM 元素），所以 debug condition 正常通过。但我在分析时花了很长时间才搞清楚到底检查的是哪个变量。

**教训**：遇到不可见字符混淆时，逐字符检查变量名。不要相信肉眼的阅读——肉眼看不出 U+FFA0，但 JavaScript 引擎能区分。用 `\uFFA0` 转义形式标注所有涉及隐藏字符的地方。

#### 机制 8：tagged template 隐式 ROT13

**我踩的坑：这是导致 20+ 个版本全错的最大元凶**

详见 [4.3 最大的错误](#43-最大的错误误删-rot13-步骤)。简单说：第 262 行 `let pool = ﾠ\`...\`;` 看起来像普通字符串赋值，实际上 `ﾠ`（U+FFA0）是 ROT13 函数名，作为 tagged template 的标签函数在隐式调用。遗漏了这步 ROT13 后，pool 中的字符全部算错，所有 flag 都包含非法字符。

**教训**：当一行代码"看起来太简单"时，检查是否有隐藏的语法机制。tagged template 是 JavaScript 中少见的"函数调用长得像字符串赋值"的语法，出题人正是利用了这种视觉欺骗。

#### 复盘总结：一周的弯路可以归纳为两句话

1. **试图绕过运行环境，而不是在运行环境中工作。** debug condition、Proxy、getter 的 step 增量都依赖 Chrome 的运行时行为。在 Python 中模拟这些行为，即使差一个 step，flag 就完全错误。最终成功的做法是：让 Chrome 自己跑，我只负责输入和输出。
