...
| 属性 | 含义 | 示例 |
|------|------|------|
| `Path` | Cookie 只在该路径及其子路径下发送 | `Path=/admin` 表示只访问 `/admin/*` 时才带这个 Cookie |
...
你可能会问：为什么是 Bot（受害者）在设置 Cookie，而不是 Next.js 应用（`http://46.62.153.171:4000/`对应的服务）在设置？这是因为这道题的设计选择：Bot 通过 Puppeteer 的 `page.setCookie()` 直接在浏览器中设置 Cookie，效果等同于服务器通过 `Set-Cookie` 响应头设置 Cookie。在实际场景中，Cookie 通常是由服务器在登录等接口中通过 `Set-Cookie` 响应头设置的，但在这道 CTF 题中，为了简化设计，直接在 Bot 的浏览器中预设了 Cookie。无论哪种方式，最终效果一样：Bot 的浏览器在访问 `proxy:4000` 时会自动带上 `flag=SK-CERT{...}` 这个 Cookie。
...
2. **nginx 部分**：`Accept-Encoding`——nginx 配置中有 `proxy_set_header Accept-Encoding ""` 或 gzip 模块会在某些配置下添加。在这道题的 nginx.conf 中没有显式添加 `Accept-Encoding` 到 Vary，但 nginx 的默认行为在启用 gzip 时会自动在 Vary 中追加 `Accept-Encoding`。你可以在实际访问 `http://46.62.153.171:4000/` 时，在 Doc 类型请求的响应头中看到完整的 Vary 值。其他类型的请求（如 JS、CSS 等静态资源）不经过 App Router，所以不会添加 Next.js 的 Vary 参数，但可能因为 nginx 的 gzip 处理而有 `Vary: Accept-Encoding`。

这个函数在每次 App Router 请求处理时都会被调用，给响应加上 Vary 头。
...
**如果 flight data 被浏览器当 HTML 解析会怎样**：正常情况下这不会发生，因为 flight data 的 Content-Type 是 `text/x-component`（不是 `text/html`），浏览器不会把它当 HTML 解析。**但如果攻击者能把 Content-Type 改成 `text/html`**，浏览器就会尝试把 flight data 的内容当 HTML 解析。此时 flight data 中类似 `"nonce":"<script>alert(1)</script>"` 的内容，浏览器会在文本中遇到 `<script>` 标签并执行它——因为浏览器只看 Content-Type 来决定如何解析，不关心内容本来是什么格式。
...
但这只是**主键（primary key）**。nginx 还会看响应中的 `Vary` 头来创建**二级键（secondary key）**。这是 **nginx 的缓存功能**（HTTP 规范定义的行为，所有符合规范的缓存代理都会这样做）。Next.js 只是在响应中设置了 Vary 头，它不知道也不关心缓存代理怎么处理。nginx 作为缓存代理，按照 HTTP 规范读取 Vary 头并据此区分缓存副本。

**主键和 Vary 二级键的关系**：它们不冲突，而是**配合使用**。可以把它理解为一个二维查找表：

```
主键: GET|http://proxy:4000/_next/pwn
  ├── 二级键: rsc="" + AE="gzip, deflate"        → 缓存副本 A
  ├── 二级键: rsc="1" + AE="gzip, deflate"        → 缓存副本 B
  └── 二级键: rsc="" + AE="gzip, deflate, br"     → 缓存副本 C

主键: GET|http://proxy:4000/_next/other
  ├── 二级键: rsc="" + AE="gzip, deflate"        → 缓存副本 D
  └── ...
```

nginx 首先用主键找到一组缓存副本，然后用当前请求的 Vary 头部值匹配二级键，找到具体的那个副本。**注意：Vary 是响应头，它出现在 nginx 从 Next.js 拿到的响应中。nginx 缓存这个响应时，会记录"当初是哪些请求头的值导致了这个响应"，后续请求只有匹配这些值才能拿到这个缓存副本。**
...
export function middleware(request: NextRequest) {
    ...
    const requestHeaders = new Headers(request.headers)  // 复制请求头（用于传递给后续处理）
...
**但如果我们同时做了两件事**：（1）让请求带 `RSC: 1` 头触发 RSC 渲染（得到未转义的 nonce），（2）利用中间件的 CT 覆盖功能把响应的 Content-Type 改成 `text/html`，浏览器就会把 flight data 的内容当 HTML 解析。此时 flight data 文本中出现的 `<script>alert(1)</script>` 被浏览器的 HTML 解析器当作真正的 `<script>` 标签，其中的 JavaScript 代码会被执行——XSS 就生效了。
...
这意味着 nginx 缓存会根据这些请求头的值来创建二级键（详见 §2.2），区分不同的缓存副本。
...
**这和 §6.1 说的"不发送 RSC 和发送空串有区别"不矛盾**：§6.1 讲的是 **Next.js**（Node.js 代码）中的判断，在 JavaScript 中 `headers['rsc'] === undefined`（不发送）和 `headers['rsc'] === ''`（发送空串）是不同的；而本节讲的是 **nginx**（C 代码）中的判断，在 nginx 中 `$http_rsc` 在两种情况下都是空字符串，不做区分。正是这个"Next.js 区分了，nginx 没区分"的差异，才是我们能绕过的根本原因。
...
**Bot 的浏览器为什么不发 RSC 头**：因为 Bot 只是简单地用 `page.goto(url)` 打开一个 URL，这和你在浏览器地址栏输入 URL 按回车一样——浏览器只发送标准的 HTTP 头（`Host`、`User-Agent`、`Accept-Encoding` 等），不会发送 `RSC` 这种自定义头。只有 Next.js 的前端 JavaScript 代码在做**客户端导航**（点击链接不刷新页面）时才会自动添加 `RSC: 1` 头。Bot 的浏览器直接打开 URL，不存在客户端导航的行为，所以没有 RSC 头。

这一点对攻击至关重要：因为 Bot 不发 RSC 头，nginx 把它当作空字符串；我们投毒时发 `RSC: ""`（空字符串），nginx 也当作空字符串；两者匹配，Bot 就能命中我们投毒的缓存。
...
**nginx 为什么会这样？** nginx 的 `proxy_cache_key` 使用 `$host` 变量，而 `$host` 的值直接来自请求的 `Host` 头。nginx 不会验证 Host 头是否和实际连接的 IP/域名一致——它只是原样使用。这在很多场景下是合理的（比如一个 nginx 同时为多个域名服务），但在这道题中，缓存键的 Host 部分可以被攻击者自由控制。

**关于浏览器中的 Host 头**：浏览器在开发者工具的 Network 面板中默认不显示 `Host` 头，因为它是 HTTP/1.1 的必需头（浏览器自动添加），不属于"自定义"头。但浏览器确实发送了它——你可以在 Chrome DevTools 中右键请求头列表 → 勾选"Show headers"或查看原始请求来确认。当你在地址栏输入 `http://46.62.153.171:4000/` 时，浏览器自动设置 `Host: 46.62.153.171:4000`。当 Bot 访问 `http://proxy:4000/xxx` 时，Bot 的浏览器自动设置 `Host: proxy:4000`。Host 的值取决于 URL 中的域名/IP 部分，可能是 IP 也可能是域名。
...
而 Vary **响应头**里包含 `Accept-Encoding`（这是服务器在响应中告诉缓存的指令），所以 nginx 会根据请求中 AE 头的值创建不同的缓存副本。具体来说，nginx 缓存响应时会记录"这个响应是在请求头 AE=gzip,deflate 时产生的"，后续只有 AE 值相同的请求才能命中这个缓存副本。如果投毒时发的 AE 和 Bot 浏览器发的 AE 不一样，缓存就不匹配。
