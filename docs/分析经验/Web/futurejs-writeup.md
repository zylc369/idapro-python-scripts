...
| `Lax`（默认值） | 大部分跨站请求不发送 Cookie，但**顶级导航**会发送。顶级导航是指：浏览器的地址栏 URL 从一个站变成了另一个站。具体包括：点击 `<a href>` 链接跳转到另一个站、用 `window.open()` 打开另一个站、提交 GET 表单到另一个站。这些操作的共同特征是**浏览器顶层窗口/标签页的 URL 发生了变化**。而像 `fetch()`、`XMLHttpRequest`、`<img>` 加载、POST 表单提交等跨站请求，Lax 模式下**不会**发送 Cookie。 |
| `None` | 允许所有跨站请求发送 Cookie，但必须同时设置 `Secure` 属性（仅 HTTPS） |

**关于安全性的疑问**：Strict 模式下跨站请求不带 Cookie，那 Lax 和 None 让跨站请求带 Cookie，会不会泄露鉴权信息？

关键在于：**Cookie 是发送给目标网站的，不是发送给来源网站的**。比如 `evil.com` 的页面中有一张图片 `<img src="https://target.com/avatar.jpg">`，浏览器会向 `target.com` 发请求并带上 `target.com` 的 Cookie——但这个请求是发给 `target.com` 的，`evil.com` 的服务器**收不到**这个请求的响应（跨域限制）。`evil.com` 只知道"图片加载成功/失败"，不知道 Cookie 的值，也看不到 `target.com` 返回了什么。

真正危险的不是"Cookie 被发送到其他网站"，而是：
- **CSRF（跨站请求伪造）**：`evil.com` 可以让浏览器**替用户操作** `target.com`（比如发一个转账请求，浏览器自动带上 Cookie），但它**看不到结果**
- SameSite=Lax 正是为了防御 CSRF——它禁止了大部分跨站请求带 Cookie，只允许"点击链接跳转"这种用户明确意图的操作带 Cookie
...
- **拦截内部页面链接**（指向同一网站其他页面的 `<a href="/about">` 类链接）：改为用 `fetch()` 发起一个带 `RSC: 1` 等自定义头的请求，获取 flight data，然后局部更新页面 DOM
...
注意：flight data 中的 `"nonce":"..."` 是 React 组件树序列化后的格式，和 layout.tsx 源码中的 `nonce={nonce}` 看起来不同，但表达的是同一个东西——`<body>` 标签的 `nonce` 属性。React 的渲染流程是这样的：

```
源代码（JSX）:  <body nonce={nonce}>
                           ↑ JSX 变量引用
...
同一个 `nonce` 值，经过不同的渲染管道，输出格式不同，是否转义也不同。这就是为什么我们需要触发 RSC 渲染——只有 flight data 模式才不会转义 `<`。
...
**我们的攻击为什么能生效？** 因为 Bot 的浏览器是**直接在地址栏访问 URL**（`page.goto(url)`），不是通过 `fetch()` 请求。地址栏访问时，浏览器**直接**处理响应：
- 如果 CT 是 `text/x-component` → 浏览器不知道这个格式 → 显示为文本或下载 → 没有安全风险
- 如果 CT 是 `text/html` → 浏览器启动 HTML 解析器 → 解析响应体中的 HTML 标签 → 遇到 `<script>` 就执行 JavaScript

所以攻击的核心是：让浏览器通过地址栏访问一个 URL，这个 URL 返回的响应的 CT 被覆盖为 `text/html`，而响应内容（flight data）中包含未转义的 `<script>`。浏览器把响应当 HTML 解析，`<script>` 被执行。中间没有任何 Next.js 前端 JS 参与拦截——因为是地址栏直接访问，不是客户端导航。
...
如果不按 RSC 模式渲染（即 RSC 头不存在时），app-render.js 会输出完整的 HTML 页面，nonce 值会被 HTML 转义（`<` → `&lt;`），XSS 无法生效。而 RSC 模式下输出的是 flight data（类似 JSON 的序列化格式），字符串值不做 HTML 转义——这不是一个"功能"，而是因为 flight data 的设计者认为它不会被浏览器当 HTML 解析，所以不需要 HTML 转义。我们的攻击正是利用了这个"不转义"。
...
**手动在网站上验证**：`http://46.62.153.171:4000/bot/visit` 是一个 HTTP API 接口，不接受浏览器直接访问（它需要 POST 请求 + JSON body）。验证流程需要工具：

1. **用 curl 或 Postman 投毒缓存**（等同于脚本的 Step 1）：
```bash
...
2. **用 curl 发送 Bot 访问请求**（等同于脚本的 Step 3）：
```bash
curl -X POST "http://46.62.153.171:4000/bot/visit" \
  -H "Content-Type: application/json" \
  -d '{"url":"http://proxy:4000/_next/pwn"}'
```

3. **等待几秒后，用 curl 读取 exfil 缓存**（等同于脚本的 Step 4）：
```bash
...
如果响应中包含 `STOLEN:flag=SK-CERT{...}`，说明 XSS 成功执行了。在浏览器中直接访问 `http://46.62.153.171:4000/_next/exfil` 也可能看到结果（但浏览器发的 Host 是 `46.62.153.171:4000`，可能和缓存主键不匹配，所以更可靠的方法是用 curl 加 `Host: proxy:4000`）。
