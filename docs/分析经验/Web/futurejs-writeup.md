...
- Lax 只允许 `<a href>` 链接点击这种**用户明确意图**的操作带 Cookie——用户点击一个链接时，是他自己决定要去那个网站，所以带 Cookie 是合理的
- 但即便是 `<a href>` 链接，evil.com 也**看不到**目标网站的响应（跨域限制），所以 Lax 模式下 evil.com 仍然无法窃取 Cookie 值
- SameSite=None 则允许所有跨站请求带 Cookie，包括 `<img>` 等——所以 None 模式下 CSRF 攻击是可能的
...
静态资源本身确实不太需要 nginx 缓存——它们变化很少，浏览器自己有本地缓存。但在这道题的配置中，`location ^~ /_next/` 覆盖了所有 `/_next/` 开头的路径（包括 `/_next/static/`），所以静态资源也会被 nginx 缓存。这意味着理论上静态资源的缓存也可以被投毒——但由于静态资源不经过 App Router 和 middleware，没有 `x-nonce` 注入点，CT 覆盖也不生效，所以实际无法利用。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ● Flight data 模式（带 RSC 头的请求）:
    React 生成类似 JSON 的序列化字符串，不做 HTML 转义：
    ["$","body",null,{"nonce":"<script>alert(1)</script>","children":[...]}]
    ↑ React 认为这是 JSON 风格的格式，不需要 HTML 转义
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ● CT = text/x-component 时 → 不启动 HTML 解析器 → 交给 JS 处理（或显示源码）
    flight data 中的 <script> 不会被当作标签 → 安全 ✓
```
...
所以攻击的核心是：让 Bot 的浏览器访问一个 URL（地址栏方式），这个 URL 返回的响应的 CT 被覆盖为 `text/html`，而响应内容（flight data）中包含未转义的 `<script>`。浏览器把响应当 HTML 解析，`<script>` 被执行。中间没有任何 Next.js 前端 JS 参与拦截——因为是地址栏直接访问，不是客户端导航。
...
④ app-render.js:
   ...
   → flight data 是 JSON 风格的序列化格式 → 不做 HTML 转义
...
⑥ nginx 收到 Next.js 的响应 → 缓存 → 后续 Bot 访问时直接返回缓存的响应
