...
- **JS 模拟点击 `<a>` 能否 CSRF？** 不能。虽然 `evil.com` 的 JavaScript 可以调用 `document.getElementById('link').click()` 来模拟点击一个 `<a href="https://bank.com/transfer?to=hacker">` 链接，浏览器确实会带 Cookie 跳转到 bank.com，但这跳转是**顶层导航**（地址栏 URL 变成了 bank.com），用户会看到自己到了 bank.com 的页面，可以发现异常。更重要的是，evil.com 的 JavaScript **无法控制跳转后的页面内容**——用户看到的是 bank.com 的真实页面，evil.com 无法注入任何恶意操作。CSRF 的关键在于"用户不知道操作发生了"，而 `<a>` 链接跳转用户是能看到的
...
但假设存在一个可以影响静态资源响应内容的"不安全输入点"（比如某个头会被反射到静态资源响应中），攻击者就可以构造一个包含恶意内容的请求，让静态资源返回被投毒的内容，缓存后影响 Bot。**在这道题中不存在这样的输入点**——静态资源是构建时生成的固定文件，不经过 middleware，不受 `x-nonce` 或 `Content-Type` 影响。所以虽然缓存覆盖了静态资源路径，但实际无法利用。这也说明了缓存投毒的核心前提：**必须有服务端代码提供的"不安全输入点"**，光有缓存是不够的。
...
而 flight data 的输出格式不是 HTML，是 React 组件树的序列化格式。因为不是 HTML，React 不做 HTML 转义——字符串值中的 `<` 保持原样。在正常使用中这完全没问题（flight data 的 CT 是 `text/x-component`，浏览器不会当 HTML 解析），但如果攻击者能让浏览器把 flight data 当 HTML 解析，未转义的 `<script>` 就会被执行。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ● HTML 模式（不带 RSC 头的普通请求）:
    React 生成 HTML 字符串，过程中会对属性值做 HTML 转义：
    <body nonce="&lt;script&gt;alert(1)&lt;/script&gt;">
    ↑ 这是 React 代码做的转义（把 < 变成 &lt;），不是浏览器做的
    ↑ 目的：防止属性值中的特殊字符被浏览器的 HTML 解析器误解为标签

  ● Flight data 模式（带 RSC 头的请求）:
    React 生成组件树的序列化字符串（flight data 格式），不是 HTML，所以不做 HTML 转义：
    ["$","body",null,{"nonce":"<script>alert(1)</script>","children":[...]}]
    ↑ 因为输出格式不是 HTML，React 不做 HTML 转义
...
flight data 里 `<` **不会转义**——因为 flight data 的输出格式不是 HTML，React 不做 HTML 转义。在正常使用中这完全没问题，因为 flight data 的 Content-Type 是 `text/x-component`，浏览器不会把它当 HTML 解析。
...
如果不按 RSC 模式渲染（即 RSC 头不存在时），app-render.js 会输出完整的 HTML 页面，nonce 值会被 HTML 转义（`<` → `&lt;`），XSS 无法生效。而 RSC 模式下输出的是 flight data（React 组件树的序列化格式），不是 HTML——因为输出格式不是 HTML，React 认为不需要做 HTML 转义。这个设计在正常情况下没问题（flight data 的 CT 是 `text/x-component`，浏览器不会当 HTML 解析），但如果攻击者能修改 CT 让浏览器把 flight data 当 HTML 解析，未转义的 `<script>` 就会被执行。
...
防御方法：统一两处的检查逻辑。两个模块应该使用相同的条件来判断是否为 RSC 请求——要么都用 `=== '1'`，要么都用 `!== undefined`：

```javascript
// 防御性修改：统一 app-render.js 的检查逻辑
const isRSCRequest = headers['rsc'] === '1';  // 和 base-server.js 一致
```
...
虽然这道题的攻击不依赖跨站 Cookie（攻击者和 Bot 访问的是同一个站点），但作为通用安全实践，鉴权类 Cookie 应该设置 `SameSite=Strict` 或 `SameSite=Lax`，减少 CSRF 风险。这道题的 Cookie 没有设置 SameSite（默认为 Lax），所以防御层面已经有一定保护。
