...
- **`$TASK_DIR` 来源**：Binary-Analysis agent 调用时已有 `$TASK_DIR`（由 agent 创建）。用户直接调用命令时，`gui-automation.md` 中应说明：先创建临时工作目录 `~/bw-ida-pro-analysis/workspace/gui_<timestamp>/`，截图和 index.json 都放在其中的 `view/` 子目录下
...
MCP 视频理解基于关键帧提取（不是逐帧分析），无法可靠判断"按钮是否被点击"这种瞬时事件。视频限制 8MB 也严重限制录屏时长。录屏不作为操作成功判断的手段。录制视频用于用户回看见方案 C。
