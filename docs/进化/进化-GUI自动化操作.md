...
MCP 返回图片像素坐标，需要换算成屏幕坐标。`gui_capture.py` 截图时同时输出元数据：
...
```json
{
  ...
  "dpi_scale": 1.25,
  "screenshot_size": [1536, 864]
}
```

换算公式：`屏幕坐标 = 图片坐标 × (屏幕尺寸 / 截图尺寸)`
...
| 期 | 平台 | 截图 | 操作 | 窗口管理 |
|----|------|------|------|---------|
| P0 | Windows | pyautogui / mss | pyautogui + pyperclip | subprocess + ctypes Win32 |
| P1 | macOS | pyautogui | pyautogui + subprocess("osascript") | subprocess("open") + AppleScript |
| P2 | Linux | pyautogui + mss | pyautogui + xdotool | xdotool + wmctrl |
...
风险: 依赖 zai-mcp-server MCP 的可用性和响应速度；依赖 pyautogui/pyperclip/mss 的安装
```
...
```
结论: 可选做
原因: 方案 A 完全覆盖 gui_verify.py 的能力。保留的收益是"标准 Win32 场景更快（2s vs 15s）"，但增加了维护成本（两套方案）。如果 MCP 超时频繁，可作为降级策略。
```
